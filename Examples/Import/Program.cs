                                      ...
                                      SymDefObject innerSymDef = null)
        {
            ...
            if (keyPub.objectAttributes.HasFlag(ObjectAttr.Decrypt))
            {
                ...
                if (keyPub.type == TpmAlgId.Symcipher)
                {
                    ...
                    //                                cipher, out ivOut);
                    //ivIn = Globs.GetRandomBytes(swSym.IVSize);
                    //ivOut = Globs.GetRandomBytes(swSym.IVSize);
                    //byte[] decrypted = swKey.Decrypt(cipher, ref ivIn, out ivOut);
                    byte[] decrypted = tpm.EncryptDecrypt(hKey, 1, TpmAlgId.Null, ivIn, cipher, out ivOut);
